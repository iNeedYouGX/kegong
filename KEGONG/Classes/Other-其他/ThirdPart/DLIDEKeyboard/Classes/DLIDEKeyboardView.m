...
+ (void)attachToTextView:(UIResponder<UITextInput> *)textView {
    ...
    if (![textView respondsToSelector:@selector(setInputAccessoryView:)]) {
        [NSException raise:@"Keyboard can be attached only to text inputs" format:@""];
    }
...
- (id)init {
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 0, SCR_WIDTH, kKeyboardHeight);
        
        UIImageView* imageBack=[[UIImageView alloc] initWithFrame:CGRectMake(SCR_WIDTH - 50, 0, 40, 40)];
        ...
        UIImageView* imageBack1=[[UIImageView alloc] initWithFrame:CGRectMake(SCR_WIDTH - 50, 37, 40, 3)];
        ...
        Button.frame = CGRectMake(SCR_WIDTH - 50, 0, 40, 40);
